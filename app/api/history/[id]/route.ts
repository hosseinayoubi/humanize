...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (!row) return NextResponse.json({ success: false, error: "Text not found", code: "TEXT_NOT_FOUND" }, { status: 404 })
    if (row.userId !== session.user.id) return NextResponse.json({ success: false, error: "Forbidden", code: "FORBIDDEN" }, { status: 403 })
